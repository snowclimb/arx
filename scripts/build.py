...
def home_page(media):
    ...
    discord = SITE_CONFIG.get("discord_url", "#")
    civmap = SITE_CONFIG.get("civmap_url", "#")
    return f'''<section class="home-hero{' has-photo' if hero else ''}"{hero_style}><div class="home-hero-overlay"></div><div class="container home-hero-inner"><div class="home-hero-copy"><div class="eyebrow light">Technocratic State of Arx</div><h1>The Westernmost<br>Civilisation</h1><p>Explore Arx, learn the mechanics of CivMC, join our community, or browse the nation’s public legal archive.</p><div class="hero-actions"><a class="hero-btn primary" href="join/index.html">Join Arx {icon('arrow')}</a><a class="hero-btn ghost" href="guides/index.html">New to CivMC?</a></div></div><div class="home-hero-mark"><img src="assets/images/arx-state-crest.webp" alt="Arx state crest"></div></div></section>
<section class="home-paths"><div class="container"><div class="section-heading"><div><div class="eyebrow">Start here</div><h2>Find your way into Arx</h2></div><p>See below the three main areas of this site.</p></div><div class="home-path-grid">
...
def about_page():
    civmap = SITE_CONFIG.get("civmap_url", "#")
    return f'''<section class="general-hero about-hero"><div class="container general-hero-inner"><div><div class="eyebrow">About Arx</div><h1>A nation built around expertise and contribution.</h1><p>The Technocratic State of Arx is a player-run nation on CivMC. We're organised around specialised departments, a constitutional framework and a strong emphasis on practical participation.</p></div><img src="../assets/images/arx-state-crest.webp" alt="Arx state crest"></div></section><div class="container about-content"><section class="about-lead"><h2>What you can find here</h2><p>This website is the public front door to Arx. Combining newcomer information, CivMC guides, media and the Arxian Justice Archive in one searchable place.</p></section><div class="about-card-grid"><a href="../join/index.html" class="about-card"><span>{icon('people')}</span><h3>Join Arx</h3><p>Learn how to enter the community and get started.</p></a><a href="../guides/index.html" class="about-card"><span>{icon('guide')}</span><h3>Learn CivMC</h3><p>Use our guided mechanic library instead of learning everything at once.</p></a><a href="../justice/index.html" class="about-card"><span>{icon('archive')}</span><h3>Justice Archive</h3><p>Browse the current Constitution and public government records.</p></a><a href="media/index.html" class="about-card"><span>{icon('image')}</span><h3>Media</h3><p>See Arx through photos, videos and promotional material.</p></a><a href="{esc(civmap)}" target="_blank" rel="noopener" class="about-card"><span>{icon('map')}</span><h3>CivMap {icon('external')}</h3><p>Open the nation-specific view on CivMap.</p></a></div></div>'''
